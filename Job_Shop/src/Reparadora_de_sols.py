    ...
    def reparacion(solucion, lista_de_vertices):
        ...
        while (planificables or (sigue_en_job or sigue_en_maquina)): 
            ...
            for operacion_idx, op_id in enumerate(planificables):
                actualizacion_de_listas(op_id, info, etiquetadas, sigue_en_job, sigue_en_maquina)
            ...
            while planificables == False: # Si `planificables` es vacío (pero aún hay operaciones en
                ...
                for operacion_idx, op_id in enumerate(maquina_elegida):
                    ...
                    sigue_en_job.remove(op_elegida_id)
                    ...
                    ### PENDIENTE: Creo que te conviene más agregar un método de actualización a las listas
                    ### de etiquetadas, etc. ###

                    # La `op_elegida` es etiquetable, entonces:
                    etiquetadas.append(operacion_elegida_id) # Agregamos cada operación planificable a la lista de
                                                             # operaciones etiquetadas.
                    
                    info[operacion_elegida_id]['completada'] = True # Marcamos a la operación elegida como completada
                    
                    break # Detiene el for después del primer intercambio, evitando que siga moviendo más 
                          # operaciones. Si todo sale bien está garantizado que |planificables| == 1
